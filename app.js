...
function applyOwnerFilter(status=ownerActiveFilter){
  ...
  document.querySelectorAll('.ownerTrip').forEach(card=>{const active=card.dataset.status!=='Archived';const okStatus=ownerActiveFilter==='All'?(active):(ownerActiveFilter==='Due'?card.dataset.due==='true':card.dataset.status===ownerActiveFilter);const okSearch=!q||card.dataset.search.includes(q);const show=okStatus&&okSearch;card.style.display=show?'':'none';if(show)visible++});
