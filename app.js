...
function fareMovement(r){
  const paid=Number(r?.paid||0),observed=Number(r?.current_price||0);
  if(!paid||!observed)return {kind:'pending',label:'No fare movement logged',amount:0,body:'Run the next same-itinerary fare check before deciding whether customer outreach is useful.'};
  const delta=paid-observed;
  if(delta>0)return {kind:'drop',label:'Lower fare logged',amount:delta,body:'Treat this as review-only until source, cabin, rules, fees, and customer approval path are verified.'};
  if(delta<0)return {kind:'rise',label:'Fare rose since booking',amount:Math.abs(delta),body:'No refund/credit path from a higher fare. Keep monitoring on schedule and do not contact the customer unless a lower eligible fare appears.'};
  return {kind:'flat',label:'Fare unchanged',amount:0,body:'No savings action yet. Keep the trip in scheduled monitoring.'};
}
...
function customerTripDeleteButton(r){
  ...
  return `<button class="btn danger" type="button" data-action="delete-trip" data-id="${escapeHtml(r.id)}" data-trip-label="${escapeHtml(label)}" aria-label="Remove RouteRefund record for ${escapeHtml(label)}">Remove trip</button>`
}
...
function customerPriceTrackingCard(r,detail=false){
  const verified=hasVerifiedFlightDetails(r);
  const archived=['Closed','Archived'].includes(r.status||'');
  if(!verified){
    return `<div class="priceTrackingCard pending" aria-label="Price tracking setup"><b>Price tracking waits for lookup</b><span>After the flight details are verified, RouteRefund can use owner-only fare alerts and backup checks for this route. You do not need to set up anything.</span></div>`;
  }
  if(archived){
    return `<div class="priceTrackingCard quiet" aria-label="Price tracking status"><b>Tracking paused</b><span>This RouteRefund record is resolved. It can be added again later without changing the airline reservation.</span></div>`;
  }
  const route=[r.route,r.travel_date].filter(Boolean).join(' • ');
...
function customerMonitoringMethods(r){
  const verified=hasVerifiedFlightDetails(r);
  const items=[
    {k:'Reservation lookup',v:verified?'Flight details verified for monitoring.':'Running first so RouteRefund does not guess route, date, or fare rules.'},
    {k:'Price-alert signals',v:verified?'Route/date can be watched with owner-only Google Flights alerts where supported.':'Starts after the reservation details are confirmed.'},
    {k:'Partner review',v:'Exact savings and airline actions stay private until RouteRefund verifies eligibility and asks for approval.'}
  ];
...
function customerWaitingRoomCard(r,detail=false){
  const verified=hasVerifiedFlightDetails(r);
  const review=['Savings found','Review needed'].includes(r.status||'');
  const archived=['Closed','Archived'].includes(r.status||'');
  const title=review?'RouteRefund is reviewing a signal':archived?'This RouteRefund record is resolved':verified?'Fare monitoring is set up':'Lookup is running';
  const body=review?'A partner is checking the source, cabin, rules, fees, and customer approval path before any next step is shared.':'This panel keeps the customer experience clear without asking you to set up alerts or share credentials.';
  const items=review?[
    ['Wait for reviewed outreach','RouteRefund will contact you only after the opportunity is verified.'],
    ['Do not change plans early','Avoid acting on an unreviewed price signal until eligibility is checked.'],
    ['Keep notes safe','You can add travel preferences, but never passwords, card numbers, or one-time codes.']
  ]:archived?[
    ['No airline action from this page','This record is retained or removed only inside RouteRefund.'],
    ['Add again if needed','You can submit the trip again later if monitoring should resume.'],
    ['Keep the reservation separate','Deleting this record does not cancel or rebook the airline trip.']
  ]:verified?[
    ['RouteRefund watches signals','Google Flights alert emails and quota-safe backup checks stay partner-controlled.'],
    ['Exact savings stay private','Potential savings are hidden until evidence is reviewed.'],
    ['Approval still required','RouteRefund asks before any airline booking, cancellation, or rebooking action.']
  ]:[
    ['Leave the reservation as-is','No airline change is being made while lookup runs.'],
    ['Watch for auto-refresh','This page updates pending trips for the first few minutes, then you can refresh manually.'],
    ['Forward the email if asked','If lookup needs help, forwarding the airline confirmation can reduce typos and missing details.']
  ];
  return `<section class="customerWaitingRoom ${detail?'detailWaitingRoom':''} ${review?'review':archived?'archived':verified?'ready':'pending'}" aria-label="What you can do now"><div><span class="eyebrow">What you can do now</span><h3>${escapeHtml(title)}</h3><p>${escapeHtml(body)}</p></div><ul>${items.map(([k,v])=>`<li><b>${escapeHtml(k)}</b><span>${escapeHtml(v)}</span></li>`).join('')}</ul></section>`;
}
...
function customerStatusFeed(r){
  ...
  const review=['Savings found','Review needed'].includes(r.status||'');
  const archived=['Closed','Archived'].includes(r.status||'');
  const submittedAt=r.created_at?formatObservedAt(r.created_at):'Saved';
  const rows=[
    {title:'Trip submitted',time:submittedAt,body:'RouteRefund saved the reservation details needed for lookup.',state:'done'},
    {title:verified?'Flight details verified':'Looking up flight',time:verified?'Complete':'In progress',body:verified?'Route, date, or departure details are available for monitoring.':'The lookup is still checking the reservation before fare tracking starts.',state:verified?'done':'active'},
    {title:verified&&!archived?'Fare tracking setup':'Fare tracking waits',time:verified&&!archived?'Ready':'Pending',body:verified&&!archived?'Owner-only Google Flights alerts and quota-safe backup checks can be used behind partner review.':'Price-alert setup starts only after the itinerary is verified.',state:verified&&!archived?'ready':'pending'},
    {title:review?'Opportunity review':'Partner review gate',time:review?'In review':archived?'Resolved':'Standing by',body:review?'A possible fare signal is being checked before customer outreach.':'Exact savings and airline actions stay private until RouteRefund verifies eligibility and asks for approval.',state:review?'review':archived?'done':'pending'}
  ];
  return `<section class="customerStatusFeed" aria-label="Trip activity and monitoring status"><div class="feedHead"><span class="eyebrow">Status feed</span><h3>What is happening with this trip</h3></div><div class="feedList">${rows.map(x=>`<div class="feedItem ${escapeHtml(x.state)}"><span aria-hidden="true"></span><div><b>${escapeHtml(x.title)}</b><small>${escapeHtml(x.time)}</small><p>${escapeHtml(x.body)}</p></div></div>`).join('')}</div></section>`;
}
...
function modal(html){
  ...
  card.innerHTML=`<button class="modalClose" type="button" data-action="close-modal" aria-label="Close dialog">×</button>${html}`;
...
function ownerNextStep(r){const label=ownerStatusLabel(r.status),movement=fareMovement(r);if(r.due_checks?.length)return 'Compare the same itinerary, record the observed fare, then schedule the next check or send to review.';if(label==='Review needed')return 'Verify evidence and customer eligibility before any customer follow-up.';if(label==='Archived')return 'Resolved and retained for reference; no active customer action.';if(movement.kind==='rise')return 'Fare rose versus customer-paid amount. No savings outreach — keep monitoring for a future drop.';if(movement.kind==='flat')return 'Fare is unchanged versus customer-paid amount. Keep scheduled monitoring.';return 'Continue monitoring until the next scheduled fare check.'}
...
async function lookupAttemptsByTrip(){const {data,error}=await supabaseClient.from('airline_lookup_attempts').select('trip_id,airline,status,attempt_count,last_error,result_excerpt,updated_at').order('updated_at',{ascending:false});if(error)return {};return (data||[]).reduce((acc,a)=>{(acc[a.trip_id] ||= []).push(a);return acc},{})}
function renderLookupAttemptCard(r,compact=false){const attempts=r.lookup_attempts||[];const latest=attempts[0];if(!latest)return `<section class="lookupAttemptCard pending ${compact?'compact':''}"><div><span>Reservation lookup</span><b>No lookup attempt yet</b><p>New customer submissions queue a supported airline lookup automatically. Unsupported carriers stay in partner review.</p></div></section>`;const status=latest.status||'Queued';const tone=({'Found':'ready','Queued':'pending','Running':'pending','Blocked':'review','Needs review':'review','Not found':'review','Unsupported':'review','Error':'review'}[status]||'pending');const body=status==='Found'?'Lookup returned itinerary page data. Verify details before fare checks.':status==='Blocked'?'Airline blocked automation or required a security challenge. Use forwarded confirmation/manual review.':status==='Unsupported'?'No safe adapter exists for this airline yet. Use forwarded confirmation/manual review.':status==='Not found'?'Booking was not found with the entered details. Check for typos or ask for forwarded confirmation.':status==='Error'?'Lookup hit an error. Review the message and retry only if safe.':status==='Running'?'Lookup worker is currently trying the airline manage-trip page.':'Lookup is queued for the worker.';return `<section class="lookupAttemptCard ${escapeHtml(tone)} ${compact?'compact':''}"><div><span>Reservation lookup</span><b>${escapeHtml(status)}${latest.attempt_count?` • ${escapeHtml(latest.attempt_count)} attempt${latest.attempt_count===1?'':'s'}`:''}</b><p>${escapeHtml(body)}</p>${latest.last_error?`<small>${escapeHtml(latest.last_error)}</small>`:''}</div>${latest.result_excerpt&&!compact?`<details><summary>Lookup excerpt</summary><p>${safeLines(latest.result_excerpt).slice(0,900)}</p></details>`:''}</section>`}
...
function ownerTripCard(r){
  ...
  return `<article class="opsTripCard ownerTrip ${statusClass}" data-status="${escapeHtml(statusLabel)}" data-due="${due?'true':'false'}" data-priority="${ownerPriority(r)}" data-search="${escapeHtml([r.confirmation_no,r.passenger_first,r.passenger_last,r.notes,r.owner_notes,r.airline,r.route,statusLabel].join(' ').toLowerCase())}"><div class="opsTripTop"><div><span class="opsPill ${statusClass}">${due?'Due now':escapeHtml(statusLabel)}</span><h3>${escapeHtml(r.airline||'Airline')} ${escapeHtml(r.confirmation_no||'')}</h3><p>${escapeHtml(r.route||'Route pending')} • ${escapeHtml(r.travel_date||'Date pending')}</p></div><a class="btn primary" href="partner-ops-trip.html?id=${encodeURIComponent(r.id)}">Open workspace</a></div><div class="opsTripMeta"><div><b>Passenger</b><span>${escapeHtml(passenger)}</span></div><div><b>Customer paid</b><span>${money(r.paid)}</span></div><div><b>Observed fare</b><span>${r.current_price?money(r.current_price):'Not logged'}</span></div><div><b>Ops timing</b><span>${escapeHtml(dueSummary)}</span></div></div><div class="opsNextAction"><b>${escapeHtml(actionHint)}</b><span>${escapeHtml(ownerNextStep(r))}</span></div>${renderLookupAttemptCard(r,true)}${renderFareMovementCard(r,true)}${renderGoogleFlightsOpsCard(r,true)}${renderBackupFareSignalCard(r,true)}${savings>0?`<div class="reviewBanner"><b>${money(savings)} potential savings</b><span>Confirm eligibility and document evidence before customer follow-up.</span></div>`:''}${r.owner_notes?`<p class="opsNote"><b>Internal note:</b> ${safeLines(r.owner_notes)}</p>`:''}<div class="opsActions">${actions}</div></article>`
}

function renderFareMovementCard(r,compact=false){
  const m=fareMovement(r);
  const amount=m.amount?money(m.amount):'No amount';
  const observed=r.current_price?money(r.current_price):'Not logged';
  const paid=r.paid?money(r.paid):'Pending';
  return `<section class="fareMovementCard ${escapeHtml(m.kind)} ${compact?'compact':''}" aria-label="Fare movement"><div><span>Fare movement</span><b>${escapeHtml(m.label)}</b><p>${escapeHtml(m.body)}</p></div><div class="fareMovementNumbers"><div><small>Paid</small><strong>${escapeHtml(paid)}</strong></div><div><small>Observed</small><strong>${escapeHtml(observed)}</strong></div><div><small>${m.kind==='drop'?'Potential savings':m.kind==='rise'?'Higher by':'Difference'}</small><strong>${escapeHtml(amount)}</strong></div></div></section>`;
}
function googleFlightsAlertState(r){
  const notes=String(r.owner_notes||'');
  const logged=/Google Flights alert/i.test(notes);
  const ready=Boolean(r.route&&r.travel_date);
  if(logged)return {label:'Alert logged',tone:'ready',body:'A partner note says Google Flights alert tracking has been set up or reviewed. Treat alert emails as owner-only signals until verified.'};
  if(ready)return {label:'Ready to set alert',tone:'ready',body:'Route and departure date are available. Set a Google Flights price alert from the separate tracking mailbox, then log the tracking URL or mailbox note here.'};
  return {label:'Waiting on details',tone:'pending',body:'Verify route and departure date before creating a Google Flights alert. Do not guess itinerary details.'};
}
function renderGoogleFlightsOpsCard(r,compact=false){
  const state=googleFlightsAlertState(r);
  const steps=compact?'':`<ol><li>Use the separate alerts/tracking mailbox, not the customer intake inbox.</li><li>Track the same route/date and comparable cabin only.</li><li>Treat alert emails as review signals; do not expose exact savings or take airline action until verified.</li></ol>`;
  const action=ownerStatusLabel(r.status)==='Archived'?'':`<button class="btn ghost" data-action="owner-google-alert" data-id="${escapeHtml(r.id)}">Log Google Flights alert</button>`;
  return `<section class="googleAlertCard ${compact?'compact':''} ${state.tone}"><div><span>Google Flights tracking</span><b>${escapeHtml(state.label)}</b><p>${escapeHtml(state.body)}</p>${steps}</div>${action}</section>`;
}
function renderBackupFareSignalCard(r,compact=false){
  const ready=Boolean(r.route&&r.travel_date);
  const status=ready?'Backup ready':'Waiting on verified itinerary';
  const body=ready?'Use SerpAPI or another paid fare source only as a partner-approved backup for this exact route/date/cabin. Respect quota caps and document the source before moving anything to review.':'Do not run paid/API fare checks until route, date, and comparable cabin are verified.';
...
async function saveOwnerGoogleAlert(id,trackingUrl='',note=''){
  ...
  if(cleanUrl&&(!/^https:\/\//i.test(cleanUrl)||!/^https:\/\/(www\.)?google\.[^\s/]+\/travel\/flights/i.test(cleanUrl))){toast('Use a Google Flights HTTPS tracking URL, or leave the URL blank and save a note.');return false}
  const {data,error:loadError}=await supabaseClient.from('owner_trip_notes').select('owner_notes').eq('trip_id',id).maybeSingle();
  if(loadError){toast(loadError.message);return false}
  const stamp=new Date().toLocaleString(undefined,{month:'short',day:'numeric',hour:'numeric',minute:'2-digit'});
  const entry=[`Google Flights alert ${stamp}: setup/review logged.`,cleanUrl?`Tracking URL: ${cleanUrl}`:'Tracking URL: not stored yet.',cleanNote?`Note: ${cleanNote}`:'Reminder: monitor alerts from the separate tracking mailbox and verify before customer follow-up.'].join('\n');
  const next=[String(data?.owner_notes||'').trim(),entry].filter(Boolean).join('\n\n');
  return saveOwnerNote(id,next)
}
async function renderOwner(){
  ...
  const lookupAttempts=await lookupAttemptsByTrip();
  (rows||[]).forEach(r=>{r.owner_notes=privateNotes[r.id]||'';r.due_checks=dueChecks[r.id]||[];r.lookup_attempts=lookupAttempts[r.id]||[]});
...
async function renderOwnerTrip(){
  ...
  r.owner_notes=privateNotes[r.id]||'';r.due_checks=dueChecks[r.id]||[];r.lookup_attempts=lookupAttempts[r.id]||[];
...
document.addEventListener('click',async e=>{
  ...
  if(action==='owner-google-alert')return modal(`<h2>Log Google Flights alert</h2><p>Use this after setting up or reviewing a Google Flights price alert from the separate RouteRefund tracking mailbox. This logs an owner-only note; it does not run SerpAPI, expose exact savings to the customer, or change the airline reservation.</p><label>Google Flights tracking URL <span class="optional">optional</span><input id="googleAlertUrl" type="url" placeholder="https://www.google.com/travel/flights/..."></label><label>Mailbox / review note <span class="optional">optional</span><textarea id="googleAlertNote" placeholder="Example: tracking@ mailbox alert created for same airline/route/date; verify manually before outreach."></textarea></label><div class="notice smallNotice"><b>Boundary:</b> Alert emails are monitoring signals only. Partner review still gates customer follow-up and any airline action.</div><button class="btn primary" data-action="save-google-alert" data-id="${escapeHtml(id)}">Save tracking note</button>`);
  if(action==='save-google-alert'){const saved=await saveOwnerGoogleAlert(id,$('googleAlertUrl').value,$('googleAlertNote').value);if(saved)closeModal();return}
  ...
  if(action==='owner-no-savings')return modal(`<h2>Record fare check</h2><p>If the fare is unchanged or higher, this schedules the next monitoring check automatically. Add enough detail that another partner can audit what was compared.</p><label>Observed current fare <span class="optional">optional</span><input id="noSavingsPrice" type="number" min="0" step="0.01" inputmode="decimal" placeholder="Leave blank if not recorded"></label><label>Check note<textarea id="noSavingsNote" placeholder="Checked same airline/route/date/cabin. Fare was not lower, so no customer outreach yet."></textarea></label><button class="btn primary" data-action="save-no-savings" data-id="${escapeHtml(id)}">Save check + schedule next</button>`);
