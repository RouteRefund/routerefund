...
function ownerControls(){return `<div class="ownerToolbar" aria-label="Operations queue controls"><div class="ownerFilters"><button class="btn ghost" data-action="owner-filter" data-status="All">Active queue</button><button class="btn ghost" data-action="owner-filter" data-status="Due">Due now</button><button class="btn ghost" data-action="owner-filter" data-status="Review needed">Review queue</button><button class="btn ghost" data-action="owner-filter" data-status="Watching">Monitoring</button><button class="btn ghost" data-action="owner-filter" data-status="Archived">Resolved archive</button></div><input id="ownerSearch" placeholder="Search passenger, locator, airline, route, or internal note" aria-label="Search operations queue"></div>`}
...
async function renderOwner(){
  ...
  const queueIntro=`<div class="opsQueueIntro"><div><h2>Operations queue</h2><p>Work left to right: due fare checks, evidence review, monitoring, then resolved archive. No customer trip is changed from this view.</p></div><span>${dueTotal?`${dueTotal} check${dueTotal===1?'':'s'} due now`:'No checks due'}</span></div>`;
