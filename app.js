...
function customerTripStatus(r){
  ...
  if(['Intake review','Received'].includes(status))return {label:'Looking up flight',step:'Lookup started',body:'Your booking was added. RouteRefund is looking up the reservation and will update this trip when flight details are found.',tone:'intake'};
...
function customerTripProgress(r,tripStatus){
  if(!hasVerifiedFlightDetails(r))return `<div class="lookupProgress"><span class="done">Submitted</span><span class="active">Looking up flight</span><span>Monitoring starts after lookup</span></div>`;
  return `<div class="miniTimeline"><span class="done">Submitted</span><span class="${['Monitoring','Savings found','Review needed','Closed','Archived'].includes(tripStatus)?'done':''}">Details verified</span><span class="${['Monitoring','Savings found','Review needed','Closed','Archived'].includes(tripStatus)?'done':''}">Monitoring</span><span class="${['Savings found','Review needed','Closed','Archived'].includes(tripStatus)?'done':''}">Review</span></div>`;
}
...
async function renderTrips(){
  ...
  box.innerHTML=rows.length?rows.map(r=>{
    ...
    return `<div class="trip customerTripCard simpleTripCard ${intake?'lookupOnlyCard':''}"><div class="row"><div><h3>${customerTripTitle(r)}</h3><p>Passenger: ${escapeHtml(r.passenger_first||'')} ${escapeHtml(r.passenger_last||'')}</p></div><span class="tag ${escapeHtml(status.tone)}">${escapeHtml(status.label)}</span></div>${customerTripMeta(r)}${customerTripProgress(r,tripStatus)}${intake?'':`<div class="customerNextStep"><b>${escapeHtml(status.step)}</b><span>${escapeHtml(status.body)}</span></div>`}${r.notes?`<p><b>Your note:</b> ${safeLines(r.notes)}</p>`:''}<div class="actions tripActionsSimple"><a class="btn primary" href="trip-detail.html?id=${encodeURIComponent(r.id)}">View details</a>${intake?'':`<button class="btn ghost" data-action="note" data-id="${escapeHtml(r.id)}">Send update</button>`}<button class="btn danger" data-action="delete-trip" data-id="${escapeHtml(r.id)}">Delete trip</button></div></div>`
...
async function deleteTrip(id){const {error}=await supabaseClient.from('trips').delete().eq('id',id);if(error)return toast(error.message);toast('Trip deleted');if(document.body.dataset.page==='trip-detail')location.href='trips.html';else await renderTrips()}
...
document.addEventListener('click',async e=>{
  ...
  if(action==='delete-trip')return modal(`<h2>Delete this trip?</h2><p>This removes the RouteRefund trip record and related lookup attempts from your testing dashboard. It does not change, cancel, or rebook anything with the airline.</p><div class="notice smallNotice"><b>Use for testing duplicates:</b> You can submit the same flight again after deleting this record.</div><div class="actions"><button class="btn danger" data-action="confirm-delete-trip" data-id="${escapeHtml(id)}">Delete trip</button><button class="btn ghost" data-action="close-modal">Cancel</button></div>`);
  if(action==='confirm-delete-trip'){closeModal();return deleteTrip(id)}
