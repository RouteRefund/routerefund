...
async function renderTripDetail(){
  ...
  box.innerHTML=`<div class="panel tripDetailCard simpleTripDetail"><div class="tripDetailHero"><div><span class="tripKicker">${escapeHtml(r.airline||'Flight')}</span><h2>Confirmation ${escapeHtml(r.confirmation_no||'')}</h2><p>${flightLine}</p></div><span class="tag ${escapeHtml(status.tone)}">${escapeHtml(status.label)}</span></div><div class="tripDetailSummary"><div><b>Passenger</b><span>${escapeHtml(r.passenger_first||'')} ${escapeHtml(r.passenger_last||'')}</span></div><div><b>Airline</b><span>${escapeHtml(r.airline||'—')}</span></div><div><b>Route</b><span>${escapeHtml(r.route||'Pending lookup')}</span></div><div><b>Departure</b><span>${escapeHtml(r.travel_date||'Pending lookup')}${r.departure_time?` • ${escapeHtml(r.departure_time)}`:''}</span></div>${r.paid!=null?`<div><b>Price paid</b><span>${money(r.paid)}</span></div>`:''}</div><div class="detailTimeline">${statuses.map((x,i)=>`<div class="${i<=active?'done':''}"><b>${i+1}</b><span>${x}</span></div>`).join('')}</div><div class="customerNextStep detailNextStep"><b>${escapeHtml(status.step)}</b><span>${escapeHtml(status.body)}</span></div>${(unverified||['Intake review','Received','Submitted'].includes(r.status))?'<div class="savingsBox lookupBox"><h3>Secure lookup running</h3><p>RouteRefund is looking up this reservation. Route, date, time, and flight details will appear here after the reservation is found.</p></div>':''}${['Savings found','Review needed'].includes(r.status)?'<div class="savingsBox"><h3>RouteRefund review in progress</h3><p>Our team is reviewing a possible fare-change signal for this booking. We will contact you only if there is a verified, customer-approved next step.</p></div>':''}${r.notes?`<div class="customerNotePreview detailNotes"><b>Your updates</b><span>${safeLines(r.notes)}</span></div>`:''}<div class="actions tripActionsSimple">${unverified?'':`<button class="btn ghost" data-action="note" data-id="${escapeHtml(r.id)}">Send update</button>`}${customerTripDeleteButton(r)}<a class="btn primary" href="trips.html">Back to dashboard</a></div></div>`
}
