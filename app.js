...
function ownerNextStep(r){const label=ownerStatusLabel(r.status);if(r.due_checks?.length)return 'Run fare check now';if(label==='Review needed')return 'Verify fare and contact customer';if(label==='Archived')return 'No active action';return 'Watching automatically'}
function ownerControls(){return `<div class="ownerToolbar"><div class="ownerFilters"><button class="btn ghost" data-action="owner-filter" data-status="All">All active</button><button class="btn ghost" data-action="owner-filter" data-status="Due">Needs check</button><button class="btn ghost" data-action="owner-filter" data-status="Review needed">Needs review</button><button class="btn ghost" data-action="owner-filter" data-status="Watching">Watching</button><button class="btn ghost" data-action="owner-filter" data-status="Archived">Archived</button></div><input id="ownerSearch" placeholder="Search airline, confirmation, passenger, note" aria-label="Search owner trips"></div>`}
...
function ownerTripCard(r){
  ...
  return `<article class="opsTripCard ownerTrip ${statusClass}" data-status="${escapeHtml(statusLabel)}" data-due="${due?'true':'false'}" data-search="${escapeHtml([r.confirmation_no,r.passenger_first,r.passenger_last,r.notes,r.owner_notes,r.airline,r.route,statusLabel].join(' ').toLowerCase())}"><div class="opsTripTop"><div><span class="opsPill ${statusClass}">${due?'Needs check':escapeHtml(statusLabel)}</span><h3>${escapeHtml(r.airline||'Airline')} ${escapeHtml(r.confirmation_no||'')}</h3><p>${escapeHtml(r.route||'Route pending')} • ${escapeHtml(r.travel_date||'Date pending')}</p></div><a class="btn primary" href="partner-ops-trip.html?id=${encodeURIComponent(r.id)}">Open</a></div><div class="opsTripMeta"><div><b>Passenger</b><span>${escapeHtml(r.passenger_first||'')} ${escapeHtml(r.passenger_last||'')}</span></div><div><b>Paid</b><span>${money(r.paid)}</span></div><div><b>Last seen</b><span>${r.current_price?money(r.current_price):'—'}</span></div><div><b>Next step</b><span>${escapeHtml(ownerNextStep(r))}</span></div></div>${savings>0?`<div class="reviewBanner"><b>${money(savings)} potential savings</b><span>Verify before contacting customer.</span></div>`:''}${r.owner_notes?`<p class="opsNote"><b>Internal:</b> ${safeLines(r.owner_notes)}</p>`:''}<div class="opsActions"><button class="btn ghost" data-action="owner-no-savings" data-id="${r.id}">Log check</button><button class="btn ghost" data-action="owner-review" data-id="${r.id}">Mark review needed</button><button class="btn ghost" data-action="owner-note" data-id="${r.id}">Note</button><button class="btn ghost" data-action="owner-status" data-id="${r.id}" data-status="Archived">Archive</button></div></article>`
}
async function renderOwner(){
  ...
  if($('kpis'))$('kpis').innerHTML=`<div class="hot"><b>${dueTotal}</b><span>Needs check</span></div><div><b>${found}</b><span>Needs review</span></div><div><b>${monitoring}</b><span>Watching</span></div><div><b>${total}</b><span>Active trips</span></div><div><b>${money(openSavings)}</b><span>Potential savings</span></div>`;
  box.innerHTML=ownerControls()+((rows||[]).length?rows.map(ownerTripCard).join(''):`<div class="empty"><h3>No customer trips yet</h3></div>`);
...
async function renderOwnerTrip(){
  ...
  const next=r.next_check_at?new Date(r.next_check_at).toLocaleString():'Not scheduled';
  box.innerHTML=`<section class="opsWorkspaceHero"><div><a class="mini" href="partner-ops-dashboard.html">← Back to ops dashboard</a><span class="opsPill ${ownerStatusClass(r.status,due)}">${due?'Needs check':escapeHtml(statusLabel)}</span><h1>${escapeHtml(r.airline||'Airline')} ${escapeHtml(r.confirmation_no||'')}</h1><p>${escapeHtml(r.route||'Route pending')} • ${escapeHtml(r.travel_date||'Date pending')}</p></div><div class="opsHeroActions"><button class="btn primary" data-action="owner-no-savings" data-id="${r.id}">Record fare check</button><button class="btn ghost" data-action="owner-review" data-id="${r.id}">Mark review needed</button></div></section>${due?`<div class="notice dueNotice"><b>${due} check due.</b> Compare the same airline, route, date and cabin. Do not contact customer until a lower eligible fare is verified.</div>`:''}<section class="opsWorkspaceGrid"><div class="opsMainPanel"><h2>Trip details</h2><div class="opsDetailGrid"><div><b>Passenger</b><span>${escapeHtml(r.passenger_first||'')} ${escapeHtml(r.passenger_last||'')}</span></div><div><b>Date of birth</b><span>${escapeHtml(r.date_of_birth||'')}</span></div><div><b>Customer paid</b><span>${money(r.paid)}</span></div><div><b>Last observed fare</b><span>${r.current_price?money(r.current_price):'Not recorded'}</span></div><div><b>Potential savings</b><span>${savings>0?money(savings):'None yet'}</span></div><div><b>Next scheduled check</b><span>${escapeHtml(next)}</span></div><div><b>Payment state</b><span>${escapeHtml(r.payment_status||'Not billed')}</span></div><div><b>Internal status</b><span>${escapeHtml(statusLabel)}</span></div></div><div class="opsTimeline"><div class="done"><b>1</b><span>Booking received</span></div><div class="${statusLabel!=='Archived'?'done':''}"><b>2</b><span>Monitoring fare</span></div><div class="${statusLabel==='Review needed'?'done':''}"><b>3</b><span>Verify opportunity</span></div><div class="${r.payment_status==='Paid'?'done':''}"><b>4</b><span>Fee captured / resolved</span></div></div></div><aside class="opsSidePanel"><h3>Recommended next action</h3><p>${escapeHtml(ownerNextStep(r))}</p><div class="opsStack"><button class="btn ghost" data-action="owner-no-savings" data-id="${r.id}">No lower fare — schedule next check</button><button class="btn primary" data-action="owner-review" data-id="${r.id}">Lower fare needs review</button><button class="btn ghost" data-action="owner-payment" data-id="${r.id}" data-status="Invoice sent">Mark invoice sent</button><button class="btn ghost" data-action="owner-payment" data-id="${r.id}" data-status="Paid">Mark fee captured</button><button class="btn ghost" data-action="owner-status" data-id="${r.id}" data-status="Archived">Archive resolved trip</button></div></aside></section><section class="panel opsNotesPanel"><h2>Notes</h2><div class="grid two"><div>${r.notes?`<p><b>Customer note</b><br>${safeLines(r.notes)}</p>`:'<p class="muted">No customer note.</p>'}</div><div>${r.owner_notes?`<p><b>Internal note</b><br>${safeLines(r.owner_notes)}</p>`:'<p class="muted">No internal note yet.</p>'}</div></div><div class="actions"><button class="btn primary" data-action="owner-note" data-id="${r.id}">Edit internal note</button></div></section>`
}
