...
async function signup(e) {
  e.preventDefault();
  const name = $('name').value.trim(), email = $('email').value.trim().toLowerCase(), password = $('password').value, password2 = $('password2').value;
...
async function forgotEmail(e) {
  e.preventDefault();
  const payload = {
    full_name: $('recoveryName').value.trim(),
    date_of_birth: $('recoveryDob').value,
    confirmation_no: $('recoveryConfirmation').value.trim().toUpperCase(),
    reply_email: $('recoveryReplyEmail').value.trim().toLowerCase(),
    status: 'New'
  };
  if (!payload.full_name || !payload.date_of_birth || !payload.reply_email) return toast('Fill out all required fields');
  const { error } = await supabaseClient.from('account_recovery_requests').insert(payload);
  if (error) return toast('Recovery request could not be saved. Make sure the latest Supabase SQL has been run.');
  e.target.reset();
  toast('Recovery request submitted');
}

async function resetPassword(e) {
  e.preventDefault();
  const email = $('email').value.trim().toLowerCase();
  const redirectTo = `${location.origin}${location.pathname.replace('reset-password.html', 'update-password.html')}`;
...
async function addTrip(e) {
  ...
  const trip = {
    user_id: user.id,
    passenger_first: $('passengerFirst').value.trim(),
    passenger_last: $('passengerLast').value.trim(),
    date_of_birth: $('dateOfBirth').value,
    flight_no: $('flightNo').value.trim().toUpperCase(),
...
async function renderOwner() {
  ...
  const total = rows?.length || 0, monitoring = rows?.filter(r => r.status === 'Monitoring').length || 0, found = rows?.filter(r => r.status === 'Savings found').length || 0;
  if ($('kpis')) $('kpis').innerHTML = `<div><b>${total}</b><span>Total trips</span></div><div><b>${monitoring}</b><span>Monitoring</span></div><div><b>${found}</b><span>Savings found</span></div>`;
  box.innerHTML = total ? rows.map(r => `<div class="trip"><div class="row"><div><h3>${escapeHtml(r.flight_no)} • ${escapeHtml(r.route)}</h3><p>${escapeHtml(r.travel_date || '')} • Paid ${money(r.paid)} • Conf. ${escapeHtml(r.confirmation_no || '')}</p><p>Passenger: ${escapeHtml(r.passenger_first || '')} ${escapeHtml(r.passenger_last || '')} • DOB: ${escapeHtml(r.date_of_birth || '')}</p><p>User: ${escapeHtml(r.user_id || '')}</p></div><span class="tag">${escapeHtml(r.status || '')}</span></div><div class="actions"><button class="btn ghost" data-action="owner-price" data-id="${r.id}">Add price found</button><button class="btn ghost" data-action="owner-status" data-id="${r.id}" data-status="Monitoring">Monitoring</button><button class="btn primary" data-action="owner-status" data-id="${r.id}" data-status="Savings found">Savings found</button><button class="btn danger" data-action="owner-status" data-id="${r.id}" data-status="Closed">Close</button></div></div>`).join('') : `<div class="empty"><h3>No customer trips yet</h3></div>`;
}
...
document.addEventListener('click', async e => {
  ...
  if (action === 'save-owner-price') { await updateTrip(id, { current_price: Number($('foundPrice').value), status: 'Savings found' }, true); $('modal').classList.remove('open'); return toast('Price saved'); }
});
