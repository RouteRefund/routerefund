...
async function deleteTrip(id){const {error}=await supabaseClient.rpc('delete_my_trip',{target_trip_id:id});if(error)return toast(error.message);toast('Trip deleted');if(document.body.dataset.page==='trip-detail')location.href='trips.html';else await renderTrips()}
...
document.addEventListener('click',async e=>{
  ...
  if(action==='delete-trip')return modal(`<h2>Delete this trip?</h2><p>This removes the trip from RouteRefund and clears related lookup/monitoring records. It does not change, cancel, or rebook anything with the airline.</p><div class="notice smallNotice"><b>Good for testing:</b> Delete duplicates here, then submit the same flight again whenever you want to retest lookup.</div><div class="actions"><button class="btn danger" data-action="confirm-delete-trip" data-id="${escapeHtml(id)}">Delete trip</button><button class="btn ghost" data-action="close-modal">Keep trip</button></div>`);
