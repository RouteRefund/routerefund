...
function ownerTripCard(r){
  ...
  return `<article class="opsTripCard ownerTrip ${statusClass}" data-status="${escapeHtml(statusLabel)}" data-due="${due?'true':'false'}" data-priority="${ownerPriority(r)}" data-search="${escapeHtml([r.confirmation_no,r.passenger_first,r.passenger_last,r.notes,r.owner_notes,r.airline,r.route,statusLabel].join(' ').toLowerCase())}"><div class="opsTripTop"><div><span class="opsPill ${statusClass}">${due?'Due now':escapeHtml(statusLabel)}</span><h3>${escapeHtml(r.airline||'Airline')} ${escapeHtml(r.confirmation_no||'')}</h3><p>${escapeHtml(r.route||'Route pending')} • ${escapeHtml(r.travel_date||'Date pending')}</p></div><a class="btn primary" href="partner-ops-trip.html?id=${encodeURIComponent(r.id)}">Open workspace</a></div><div class="opsTripMeta"><div><b>Passenger</b><span>${escapeHtml(passenger)}</span></div><div><b>Customer paid</b><span>${money(r.paid)}</span></div><div><b>Observed fare</b><span>${r.current_price?money(r.current_price):'Not logged'}</span></div><div><b>Ops timing</b><span>${escapeHtml(dueSummary)}</span></div></div><div class="opsNextAction"><b>${escapeHtml(actionHint)}</b><span>${escapeHtml(ownerNextStep(r))}</span></div>${renderGoogleFlightsOpsCard(r,true)}${savings>0?`<div class="reviewBanner"><b>${money(savings)} potential savings</b><span>Confirm eligibility and document evidence before customer follow-up.</span></div>`:''}${r.owner_notes?`<p class="opsNote"><b>Internal note:</b> ${safeLines(r.owner_notes)}</p>`:''}<div class="opsActions">${actions}</div></article>`
}
