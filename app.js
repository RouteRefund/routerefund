...
function customerTripStatus(r){
  ...
  if(['Intake review','Received'].includes(status))return {label:'Flight lookup running',step:'Secure lookup started',body:'Your booking was added. RouteRefund is matching the confirmation number to the airline itinerary and will update this trip when the flight details are found.',tone:'intake'};
...
function customerTripMeta(r){
  if(!hasVerifiedFlightDetails(r))return `<div class="lookupSummary compactLookup" aria-label="Flight lookup status"><div><b>Matching itinerary</b><span>Confirmation ${escapeHtml(r.confirmation_no||'')} is saved. RouteRefund is checking for the airline itinerary tied to this passenger.</span></div></div>`;
...
function customerTripProgress(r,tripStatus){
  if(!hasVerifiedFlightDetails(r))return `<div class="lookupProgress"><span class="done">Submitted</span><span class="active">Matching itinerary</span><span>Monitoring starts after match</span></div>`;
