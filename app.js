...
const PARTNER_EMAIL_ALLOWLIST=new Set(['andrew@routerefund.com','caleb@routerefund.com','max@routerefund.com']);
...
async function login(e){e.preventDefault();const email=$('email').value.trim().toLowerCase(),password=$('password').value;const partnerLogin=isPartnerLoginPage();if(partnerLogin&&!PARTNER_EMAIL_ALLOWLIST.has(email))return toast('Partner portal is restricted to approved RouteRefund partner emails. Use the customer login for your trips.');const {data,error}=await supabaseClient.auth.signInWithPassword({email,password});if(error)return toast(error.message);touchActivity();if(partnerLogin){const {data:isOwner,error:ownerError}=await supabaseClient.rpc('current_user_is_owner');if(ownerError||isOwner!==true){await supabaseClient.auth.signOut();return toast('Partner access denied for this email.')}}if(data?.user)await ensureProfile(data.user);const next=new URLSearchParams(location.search).get('next')||(partnerLogin?'partner-ops-dashboard.html':'trips.html');location.href=next}
...
async function completeMonitoringCheck(id,observedPrice,kind='No savings',note=''){
  ...
  const savingsFound=kind==='Savings found'||(price&&trip?.paid&&price<Number(trip.paid));
  await supabaseClient.from('monitoring_checks').update({checked_at:now.toISOString(),observed_price:price,result:savingsFound?'Savings found':'No savings',notes:note||'Owner completed fare check'}).eq('trip_id',id).eq('result','Due');
  await supabaseClient.from('monitoring_checks').insert({trip_id:id,check_due_at:next.toISOString(),source:'Scheduled follow-up',result:'Due',notes:'Next automated monitoring reminder'});
  const patch={last_checked_at:now.toISOString(),next_check_at:next.toISOString(),status:savingsFound?'Savings found':'Monitoring'};
  if(price)patch.current_price=price;
  const {error}=await supabaseClient.from('trips').update(patch).eq('id',id);
  if(error)return toast(error.message);
  await renderOwner();await renderOwnerTrip();toast(savingsFound?'Savings marked for review':'No savings recorded; next check scheduled')
}

function ownerControls(){return `<div class="ownerControls"><button class="btn ghost" data-action="owner-filter" data-status="All">All</button><button class="btn ghost" data-action="owner-filter" data-status="Due">Due checks</button><button class="btn ghost" data-action="owner-filter" data-status="Monitoring">Monitoring</button><button class="btn ghost" data-action="owner-filter" data-status="Savings found">Savings found</button><button class="btn ghost" data-action="owner-filter" data-status="Closed">Closed</button><input id="ownerSearch" placeholder="Search confirmation, name, note" aria-label="Search owner trips"></div>`}
...
function ownerTripCard(r){
  ...
  return `<div class="trip ownerTrip" data-status="${escapeHtml(r.status||'Monitoring')}" data-due="${due?'true':'false'}" data-search="${escapeHtml([r.confirmation_no,r.passenger_first,r.passenger_last,r.notes,r.owner_notes,r.airline,r.route].join(' ').toLowerCase())}"><div class="row"><div><h3>${due?'⚠️ ':''}${escapeHtml(r.airline||'Airline')} ${escapeHtml(r.confirmation_no||'')}</h3><p>${escapeHtml(r.route||'Route pending')} • ${escapeHtml(r.travel_date||'Date pending')}</p><p>Paid ${money(r.paid)}${r.current_price?` • Last seen ${money(r.current_price)} • Savings ${money(savings)}`:''}</p><p>Passenger: ${escapeHtml(r.passenger_first||'')} ${escapeHtml(r.passenger_last||'')} • DOB: ${escapeHtml(r.date_of_birth||'')}</p><p>Payment: ${escapeHtml(r.payment_status||'Not billed')} • Next check: ${escapeHtml(next)} ${due?`• <b>${due} due</b>`:''}</p>${r.notes?`<p><b>Customer note:</b> ${safeLines(r.notes)}</p>`:''}${r.owner_notes?`<p><b>Owner note:</b> ${safeLines(r.owner_notes)}</p>`:''}</div><span class="tag">${due?'Due check':escapeHtml(r.status||'Monitoring')}</span></div><div class="actions"><a class="btn primary" href="partner-ops-trip.html?id=${encodeURIComponent(r.id)}">Open workspace</a><button class="btn primary" data-action="owner-price" data-id="${r.id}">Lower price found</button><button class="btn ghost" data-action="owner-no-savings" data-id="${r.id}">No savings</button><button class="btn ghost" data-action="owner-note" data-id="${r.id}">Owner note</button><button class="btn danger" data-action="owner-status" data-id="${r.id}" data-status="Closed">Close</button></div></div>`
}
async function renderOwner(){
  ...
  const total=rows?.length||0,monitoring=rows?.filter(r=>r.status==='Monitoring').length||0,found=rows?.filter(r=>r.status==='Savings found').length||0,dueTotal=(rows||[]).reduce((sum,r)=>sum+(r.due_checks?.length||0),0),openSavings=(rows||[]).reduce((sum,r)=>sum+Math.max(tripSavings(r),0),0);
  if($('kpis'))$('kpis').innerHTML=`<div><b>${total}</b><span>Total trips</span></div><div><b>${dueTotal}</b><span>Due checks</span></div><div><b>${monitoring}</b><span>Monitoring</span></div><div><b>${found}</b><span>Savings found</span></div><div><b>${money(openSavings)}</b><span>Potential savings</span></div>`;
...
async function renderOwnerTrip(){
  ...
  const {data:r,error}=await supabaseClient.from('trips').select('*').eq('id',id).single();
  if(error||!r){box.innerHTML=`<div class="panel"><h2>Access blocked or trip missing</h2><p>${escapeHtml(error?.message||'This trip could not be loaded.')}</p><a class="btn primary" href="partner-ops-dashboard.html">Owner dashboard</a></div>`;return}
  const privateNotes=await ownerNotesByTrip();
  const dueChecks=await dueChecksByTrip();
  r.owner_notes=privateNotes[r.id]||'';
  r.due_checks=dueChecks[r.id]||[];
  const dueMarkup=r.due_checks.length?`<div class="notice dueNotice"><b>${r.due_checks.length} fare check due.</b> Open the airline/fare source, compare the same itinerary/cabin, then record lower price or no savings.</div>`:'';
  const savings=tripSavings(r);
  box.innerHTML=`<section class="section"><span class="eyebrow">Owner workspace</span><h1>Confirmation ${escapeHtml(r.confirmation_no||'')}</h1><p class="lead small">${escapeHtml(r.airline||'Airline')} • ${escapeHtml(r.route||'Route pending')} • ${escapeHtml(r.travel_date||'Date pending')}<br>Passenger: ${escapeHtml(r.passenger_first||'')} ${escapeHtml(r.passenger_last||'')} • DOB: ${escapeHtml(r.date_of_birth||'')} • User ID: ${escapeHtml(r.user_id||'')}</p></section>${dueMarkup}<section class="grid three section"><div class="card"><h3>Fare snapshot</h3><p>Paid: ${money(r.paid)}</p><p>Current found price: ${r.current_price?money(r.current_price):'Not added'}</p><p>Potential savings: ${money(Math.max(savings,0))}</p></div><div class="card"><h3>Status</h3><p><span class="tag">${escapeHtml(r.status||'Monitoring')}</span></p><p>Payment: ${escapeHtml(r.payment_status||'Not billed')}</p></div><div class="card"><h3>Checklist</h3><ul class="checklist"><li>Confirm booking details</li><li>Check same itinerary/cabin</li><li>Record lower price proof</li><li>Contact customer before action if needed</li></ul></div></section><section class="panel"><h2>Notes</h2>${r.notes?`<p><b>Customer:</b><br>${safeLines(r.notes)}</p>`:'<p class="muted">No customer note.</p>'}${r.owner_notes?`<p><b>Owner:</b><br>${safeLines(r.owner_notes)}</p>`:'<p class="muted">No owner note yet.</p>'}<div class="actions"><button class="btn primary" data-action="owner-price" data-id="${r.id}">Lower price found</button><button class="btn ghost" data-action="owner-no-savings" data-id="${r.id}">No savings</button><button class="btn ghost" data-action="owner-note" data-id="${r.id}">Edit owner note</button><button class="btn ghost" data-action="owner-payment" data-id="${r.id}" data-status="Not billed">Not billed</button><button class="btn ghost" data-action="owner-payment" data-id="${r.id}" data-status="Invoice sent">Invoice sent</button><button class="btn primary" data-action="owner-payment" data-id="${r.id}" data-status="Paid">Paid</button><button class="btn primary" data-action="owner-status" data-id="${r.id}" data-status="Savings found">Savings found</button><button class="btn danger" data-action="owner-status" data-id="${r.id}" data-status="Closed">Close</button><a class="btn ghost" href="partner-ops-dashboard.html">Back</a></div></section>`
}
function applyOwnerFilter(status='All'){
  const q=($('ownerSearch')?.value||'').trim().toLowerCase();
  document.querySelectorAll('.ownerTrip').forEach(card=>{const okStatus=status==='All'||(status==='Due'?card.dataset.due==='true':card.dataset.status===status);const okSearch=!q||card.dataset.search.includes(q);card.style.display=okStatus&&okSearch?'':'none'})
}
...
document.addEventListener('click',async e=>{
  ...
  if(action==='owner-price')return modal(`<h2>Lower price found</h2><label>Observed lower price<input id="foundPrice" type="number" min="0" step="0.01" placeholder="299.00"></label><label>Check note<textarea id="priceNote" placeholder="Source, same cabin/route, screenshot saved, customer action needed..."></textarea></label><button class="btn primary" data-action="save-owner-price" data-id="${id}">Save lower price</button>`);
  if(action==='owner-no-savings')return modal(`<h2>No savings this check</h2><label>Observed current price <span class="optional">optional</span><input id="noSavingsPrice" type="number" min="0" step="0.01" placeholder="Leave blank if not recorded"></label><label>Check note<textarea id="noSavingsNote" placeholder="Checked same airline/route/date/cabin. No lower eligible fare found."></textarea></label><button class="btn primary" data-action="save-no-savings" data-id="${id}">Schedule next check</button>`);
  if(action==='save-owner-price'){if(!$('foundPrice').value)return toast('Enter the lower price found.');await completeMonitoringCheck(id,$('foundPrice').value,'Savings found',$('priceNote').value.trim());$('modal').classList.remove('open');return}
